...
#include <original_arm/publish.hpp>
#include <original_arm/subscribe.hpp>
...
int main(int argc, char * argv[])
{
  ...
  const auto publish = std::make_shared<original_arm::Publish>(rclcpp::NodeOptions());
  // Subscribeノードの実態をインスタンス化し、その共有ポインタを取得
  const auto subscribe = std::make_shared<original_arm::Subscribe>(rclcpp::NodeOptions());
  // Executorに各ノードを登録する。
  // exec.add_node(std::make_shared<original_arm::Publish>(rclcpp::NodeOptions()));
